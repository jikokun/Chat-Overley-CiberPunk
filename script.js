...
// Parámetros de la API perimetral pública global de Kick
const KICK_WS_URL = "wss://ws-user.kick.com/app/eb1d5f283081a78b93bb?protocol=7&client=js&version=7.4.0&flash=false";
let socket;
let pingInterval;

function conectarChatNativo() {
    // Abrimos un WebSocket local puro sin librerías externas
    socket = new WebSocket(KICK_WS_URL);

    socket.onopen = () => {
        console.log("Conectado con éxito a la infraestructura de Kick.");
        
        // 1. Enviamos el formato manual que exige el clúster de Kick para dar acceso
        ...
        // 2. Sistema de Latido (Heartbeat): Evita que Kick nos tumbe la conexión por inactividad
        clearInterval(pingInterval);
        pingInterval = setInterval(() => {
            if (socket.readyState === WebSocket.OPEN) {
                socket.send(JSON.stringify({ "event": "pusher:ping", "data": {} }));
            }
        }, 25000); // Enviamos un ping cada 25 segundos
    ...
    socket.onmessage = (event) => {
        try {
            ...
            // Si el servidor de Kick responde a nuestro latido, mantenemos la tubería abierta
            if (rawPayload.event === "pusher:pong") return;

            // Al interceptar un mensaje real enviado en tu transmisión de Kick
            ...
            console.error("Error decodificando el paquete de Kick:", error);
        }
    };

    socket.onclose = () => {
        clearInterval(pingInterval);
        showSystemStatus("⚠️ Conexión perdida. Reconectando...");
        // Intentar reconectar automáticamente en 5 segundos si se cae la red
        setTimeout(conectarChatNativo, 5000);
    };
}
...
function processMessage(data) {
    ...
    // Filtro anti-bots incorporado
    if (botsIgnorados.includes(usernameLower) || sender.is_bot) return;

    // Extracción o autogeneración de color neón elegante para el cristal
    ...
    // Parseador automático de Emotes oficiales de Kick
    ...
    // Mapeo de insignias tradicionales (Badges)
    let badgesHtml = '';
    ...
    // Límite de rendimiento en pantalla (Máximo 4 mensajes simultáneos)
    while (chatContainer.children.length > 4) {
        chatContainer.removeChild(chatContainer.firstChild);
    }

    // Efecto de evaporación líquida neón a los 30 segundos
    setTimeout(() => {
...
// Iniciar el motor de red asíncrono puro
conectarChatNativo();